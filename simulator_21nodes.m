...
[WEE,WIE,WEI,WII,WEX,WIX,gwE,gwI]=gen_network(N,g);
    ...
    for t=1:length(input)-1
        ...
        % plasticity (WEE & WEI plasticity for resonance at 1st layer) 
        if p.resonance.on && t>p.resonance.memoryLength 
             %-------fading window-------
             fadingWindow=linspace(0,1,p.resonance.memoryLength);  
             %-------get correlation matrix (from E to E, symmetric) for Wee-------
             tmpu=mu(:,t-p.resonance.memoryLength+1:t);
             tmpu=tmpu-repmat(mean(tmpu,2),[1,size(tmpu,2)]); % remove mean before applying the fading window
             tmpu=tmpu.*repmat(fadingWindow,N,1);
             covMatrixu=cov(tmpu');
             covMatrixu=covMatrixu./max(abs(covMatrixu(:)));
             %-------get correlation matrix (from E to I, asymmetric) for Wie-------
             % covariance(E-to-I) is very similar to covariance(E-to-E) , so just use covariance(E-to-E) to save time
             %          tmpv=mv(:,t-g.learningRule.memoryLength+1:t);
             %          tmpv=tmpv-repmat(mean(tmpv,2),[1,size(tmpv,2)]);
             %          tmpv=tmpv.*repmat(fadingWindow,N,1);
             %          corrMatrixuv=corr(tmpu',tmpv');
             %-------change of WEE & WEI according to covariance-------
             dWEE=(-WEE + p.resonance.WEE_rate*abs(covMatrixu.*(covMatrixu> 0)).*gwE)/1000; %  > quantile(covMatrixu(:),0.75)
             dWEE(eye(N)==1)=0; dWEE(:,N)=0; dWEE(N,:)=0; % fix diagnal part, and fix inter-layer connection 
             WEE=max(WEE+dWEE,0);
             dWEI=(-WEI + p.resonance.WEI_rate*abs(covMatrixu.*(covMatrixu< 0)).*gwI)/1000; %  < quantile(covMatrixu(:),0.25) 
             dWEI(eye(N)==1)=0;dWEI(:,N)=0; dWEI(N,:)=0; % fix diagnal part, and fix inter-layer connection % fix diagnal part
             WEI=max(WEI+dWEI,0);   
        end
    end 
    %-------------------calculate MEG-----------------------------
...
function p=initialize_p()
    %---------plasticity (adaptive WEE & WEI for sustained resonance)-----------
    p.resonance.on = 1; % 1:ON, 0:OFF 
    p.resonance.memoryLength=1000; %200 timepoints window for correlation
    p.resonance.WEE_rate=0.05; %learning rate
    p.resonance.WEI_rate=0.05; %learning rate
    if p.resonance.on
        disp('WEE & WEI plasticity for resonance at 1st layer ON'); 
    else
        disp('WEE & WEI plasticity for resonance at 1st layer OFF'); 
    end
end
%==========================================================================  
...
%   gwE [NxN]   to be applied to WEE plasticity at the 1st layer
%   gwI [NxN]   to be applied to WIE plasticity at the 1st layer
    ifirstLayer=[1:N-1]; % index of first-layer nodes
    isecondLayer=N;      % index of second-layer nodes
...
%==========================================================================
% gausian weight
function net=gen_gausian_network(Nf,sigma)
    net=zeros(Nf,Nf);
    for i=1:Nf
        x=abs([1:Nf]-i); %not circular
        tmp=exp(-x.^2/(2*sigma^2)); 
        net(i,:)=tmp/max(tmp);
    end
end
%==========================================================================  
